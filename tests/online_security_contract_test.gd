...
const CONTROL_SERVER_PATH := "res://backend/rivet-control/src/server.ts"
...
func _initialize() -> void:
	...
	for forbidden in ["sbp_", "sb_secret_", "service_role", "cloud.eyJ"]:
		if transport.contains(forbidden):
			failures.append("Secret-like value leaked into transport source")
